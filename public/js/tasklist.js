...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (listOfTasks) {
        listOfTasks.forEach((task, index) => {
            ...
            // })
            del_buttons = document.querySelectorAll("button.delete");
            // console.log(del_buttons);
            del_buttons.forEach((button) => {
                button.addEventListener('click', () => {
                    // console.log("deleters");
                    task_el_remove = button.parentElement.parentElement;
                    list_el.removeChild(task_el_remove);
                    // console.log(button.parentElement.parentElement.firstChild.firstChild.value);
                    // localStorage.setItem("task-list", JSON.stringify(listOfTasks.slice(task_el.firstElementChild.firstElementChild.id, 1)));
                    // console.log(listOfTasks)
                    let ind;
                    listOfTasks.forEach((task, index) => {
                        if (task.taskName === button.parentElement.parentElement.firstChild.firstChild.value){
                            ind = index;
                        };
                    })
    ...
    form.addEventListener('submit', (e) => {
        ...
        del_buttons.forEach((button) => {
            button.addEventListener('click', () => {
                // console.log("deleters");
                let task_el_remove = button.parentElement.parentElement;
                list_el.removeChild(task_el_remove);
                // console.log(button.parentElement.parentElement.firstChild.firstChild.value);
                // localStorage.setItem("task-list", JSON.stringify(listOfTasks.slice(task_el.firstElementChild.firstElementChild.id, 1)));
                // console.log(listOfTasks)
...
//   del_buttons = document.querySelectorAll("#delete");
//   console.log(del_buttons);
//   del_buttons.forEach((button) => {
//     button.addEventListener('click',() => {
//         console.log("deleters");
